...
def _upload_and_select(slot: str, label: str):
    ...
    uploaded = st.file_uploader(
        label, type=["xlsx", "xls", "csv", "pdf"], key=f"uploader_{slot}"
    )
    if uploaded is None:
        return None, None
    ...
    with st.expander(f"Preview: {uploaded.name} ({df.shape[0]} rows x {df.shape[1]} cols)"):
...
def main() -> None:
    ...
    st.caption(
        "Upload two files, map matching fields, choose a primary key, and get an "
        "instant reconciliation dashboard with an exportable Excel report."
    )
    ...
    st.caption(
        "Fields with identical names are auto-mapped. Adjust any dropdown below to "
        "map fields with different names (e.g. 'Invoice No' to 'Inv_Num')."
    )
    ...
    with map_col_1:
        st.markdown("**File 1 Field**")
    with map_col_2:
        st.markdown("**Maps to File 2 Field**")
    with map_col_3:
        st.markdown("**Match Type**")

    for c1 in cols_1:
        row_1, row_2, row_3 = st.columns([2, 2, 1])
        ...
        with row_3:
            if choice == not_mapped_option:
                ...
                st.caption(auto_type_dict.get(c1, "manual"))
            else:
                st.caption("manual")
    ...
    st.divider()
    st.subheader("3. Primary Key Selection")
    st.caption("Choose at least one mapped field to uniquely identify and merge records.")
    field_options = [_field_label(m.field_1, m.field_2) for m in mapped_fields]
    pk_selection = st.multiselect("Primary Key field(s)", field_options, key="pk_selection")
